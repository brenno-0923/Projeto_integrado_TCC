...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['servico'], $_GET['id_cliente'])) {
        $servico = $_GET['servico'];
        $id_cliente = $_GET['id_cliente'];

       
        echo "ID Cliente: $id_cliente, Serviço: $servico<br>";

        
        $sql = "UPDATE cliente SET servico = ? WHERE id_cliente = ?";
        $stmt = $conexao->prepare($sql);

        
        if ($stmt) {
            $stmt->bind_param("si", $servico, $id_cliente);
...
<form method="GET" action="servicos1.php">
...
<h2>Tipos de serviços disponiveis:</h2>

<ol>
    <li><h4>Lavagem</h4></li>
    <ul>
        <li><b>Simples: </b>o veículo será limpo apenas na parte externa de forma rápida.</li>
        <li><b>Interna: </b> será limpa apenas a parte interna do veículo.</li>
        <li><b>Detalhada: </b> a parte externa e interna serão limpas com o maior cuidado pelos nossos profissionais.</li>
      </ul>
    <li>
      <h4>Pintura</h4>
      <ul>
        <li>Para obter o esse serviço, basta informar o nome da peça e a cor que desja ser pintada.</li>
        <li>Exemplo: Capô / cinza chumbo </li>
      </ul>
    </li>
    <li><h4>Manutenção</h4></li>
    <ul>
        <li>Balanceamento</li>
...
label {
  display: block; /* Faz com que o rótulo (label) ocupe uma linha inteira */
  margin-bottom: 5px; /* Adiciona um espaço entre o rótulo e a caixa de texto */
}
input[type="text"] {
  width: 10px; /* Define a largura da caixa de texto */

}
ol {
  text-align: left; /* Define o alinhamento do texto da lista para a esquerda */
}
li {
  margin: 5px 0; /* Adiciona um espaço entre os itens da lista */
}
    </style>
